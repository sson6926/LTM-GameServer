...
public class InviteContinueNextGameResponseHandler implements ActionHandler {
    ...
    public void handle(JSONObject request, ServerThread thread) {
        if (request.optBoolean("accepted")) {  
            ...
            int matchId = request.getInt("matchId");
            RoundDAO roundDAO = new RoundDAO();
            roundDAO.save(matchId);
            ...
            responseToUser.put("action", "CONTINUE_NEXT_GAME");
            ...
            responseJson.put("action", "CONTINUE_NEXT_GAME_REJECTED");
